   ...
   , _total_latency(SubsecondTime::Zero())
{
   ...
   try
   {
      ...
      _hopLatency = ComponentLatency(Sim()->getDvfsManager()->getCoreDomain(net->getCore()->getId()), Sim()->getCfg()->getInt("network/emesh_hop_counter/hop_latency")); //getDvfsManager()->getCoreDomain(net->getCore()->getId())
   }
   catch (...)
   {
