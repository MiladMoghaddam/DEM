...
class PeriodicStats:
  def setup(self, args):
    ...
    #############################
    self.filename_DNN_IN="DNN_IN.csv"
    self.Wfile_DNN_IN = file(os.path.join(sim.config.output_dir, self.filename_DNN_IN), 'w')
    self.Wfile_DNN_IN.write("feature,feature,feature,feature,feature,feature,feature,...,feature,feature,feature")
    ...
    for core in range(sim.config.ncores):
        self.Wfile_SummaryDVFS.write("\t\t%d\tFrequency\tInsDone\tPredictedInsDone\tIns_ref\tPredcited_ref\tActualLoss\tPredictedLoss\tActualLossRate\tPredictedLossRate\tTotalActualInst\tTotalPredictedIns\tTotalActualMissRate\tTotalPredictedMissRate\tPF_time\tPF_pred_time\t"%core)
...
#          else: self.Wfile_CPI.write("\t%f"%(1/(int(CoreIns[core])/((sim.dvfs.get_frequency(core)/1000.0)*(self.interval/1000000)))))
      ...
      '''
      make DNN_input.csv
              contains   stats for each core

                         core0,stat1,stat2,stat3,stat4,stat5,...statN
                         core1,stat1,stat2,stat3,stat4,stat5,...statN
                          ...
                         core2,stat1,stat2,stat3,stat4,stat5,...statN

      # run DNN.py on DNN_input.csv
      '''
      ...
      if self.DVFS:
          ...
          ########################################################################################
          ############## DNN_IN ##############################################################
          self.Wfile_DNN_IN = file(os.path.join(sim.config.output_dir, self.filename_DNN_IN), 'a')
          self.Wfile_DNN_IN.write('\n')
          for core in range(sim.config.ncores):
               self.Wfile_DNN_IN.write("%d,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s"%(core,sim.dvfs.get_frequency(core),self.InstDone[core],\
                          self.PredictedInstDone[core],self.Inst_for_freq_H[core],self.PredictedInst_for_freq_H[core],self.InstLoss[core],\
                          self.PredictedInstLoss[core],self.InstLossRate[core],self.PredictedInstLossRate[core],self.TotallInstDone[core],\
                          self.PredictedTotalInstDone[core],self.TotalInstLossRate[core],self.PredictedTotalInstLossRate[core],self.PF[core],self.PF_next[core]))          


          self.Wfile_DNN_IN.close()
          ########################################################################################
