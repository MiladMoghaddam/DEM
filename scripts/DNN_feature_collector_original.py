...
def get_stats(line_count,benchmark_dir,num_core,mode,mode_param):
              #online_freq just works with collect_online and for collect_offline, the freq is read from SummaryDVFS.txt 
              # if mode = 'OFFLINE'
              #     mode_param contains line_splitted
              # if mode = 'ONLINE'
              #     mode_param contains freqs      
              ...
              for core in range(0,num_core):
                  ...
                  if (mode == 'OFFLINE'):
                     ...
                     freq[core] = float(mode_param[2*core+2])
                     freq_class[core] = UL.translate_freq_to_class(int(freq[core]))
                  elif (mode == 'ONLINE'):
                     freq[core]=mode_param[core]
                     freq_class[core] = UL.translate_freq_to_class(int(freq[core]))
