...
#sys.path.insert(0, '/home/milad/sniper/scripts/sim')
...
def generate_simout(jobid = None, resultsdir = None, partial = None, output = sys.stdout, silent = False):
  ...
  #############################
  filename_CashMiss="CacheMiss.xls"
  if partial :
    if "periodic-0" in partial:
       Wfile_CashMiss = file(os.path.join(resultsdir, filename_CashMiss), 'w')
       Wfile_CashMiss.write("%d"%ncores)
       for core in range(ncores):
           Wfile_CashMiss.write("\tCore")
           for c in existcaches:
               Wfile_CashMiss.write("\t%s\tmisses"%c)
       Wfile_CashMiss.write("\n")
      
       for core in range(ncores):
           Wfile_CashMiss.write("\t%d"%core)
           for c in existcaches:
               Wfile_CashMiss.write("\t%s\t%s"%(results['%s.missrate'%c][core],results['%s.misses'%c][core]))
       Wfile_CashMiss.close()
    else:
       Wfile_CashMiss = file(os.path.join(resultsdir, filename_CashMiss), 'a')  
       Wfile_CashMiss.write("\n")
       for core in range(ncores):
           Wfile_CashMiss.write("\t%d"%core)
           for c in existcaches:
